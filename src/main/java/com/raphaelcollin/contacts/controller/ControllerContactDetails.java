...
public class ControllerContactDetails extends Controller implements Initializable{
    ...
    private static final String CLASS_SAVE_BUTTON =  "blue-button";
    ...
    private static final String BUNDLE_KEY_ALERT1_ERROR_HEADER_TEXT = "contact_details_alert_header_text";
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        iconContactButton.setGraphic(new FontIcon("far-id-card:64:blue"));
        ...
        saveButton.setPrefWidth(bottomButtonsWidth);
        saveButton.setFont(new Font(bottomButtonFontSize));
        saveButton.setPrefHeight(bottomButtonHeight);

        clearButton.getStyleClass().add(CLASS_CLEAR_BUTTON);
        saveButton.getStyleClass().add(CLASS_SAVE_BUTTON);
    ...
    public void handleSave(){
        ...
        try {
            Contact newContact = controllerContactFields.buildContactFromFields();
            Task<Boolean> task = new Task<>() {
                @Override
                protected Boolean call() {
                    root.setCursor(Cursor.WAIT);
                    DAO<Contact> dao = DAOFactory.getContactDAO();
                    return dao.update(oldContact.getIdContact(), newContact);
                }
            };
    ...
    void inicializarCampos(){
        controllerContactFields.setFieldsText(oldContact);
    }

    void setOldContact(Contact oldContact) {
